...
| `FORTRESS_DB_APP_NAME` | no | Label every connection with `application_name` = `hx-fortress:<role>/<instance>` (default on; `off` disables). Roles: `rw` live ingest, `ro` reads, `bg` background repair, `embed`, `purge`, `probe`. Deliberately does NOT ride the `FORTRESS_DB_STATEMENT_TIMEOUT_MS=0` pooler hatch — `application_name` is standard and a pooler-fronted deployment is where anonymous connections hurt most. Without this, `pg_stat_activity` cannot attribute load between the five pools, or between two fortresses during a rolling deploy. |
