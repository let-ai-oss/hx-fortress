...
//      text present, not soft-deleted) with NO vector yet. So a growing session
//      embeds only its new turns; a whole-session recompute never happens.
//   2. SCRUB each turn, hash the scrubbed text (content_hash).
//   3. REUSE — any content_hash already embedded is copied, skipping OpenAI.
...
//      held lease under a single in-process worker, so we don't hold one).
//   5. INSERT … ON CONFLICT (owner_kind, owner_id) DO NOTHING — the UNIQUE index
//      (0010) is the crash-/double-claim fence the anti-join alone can't provide.
//
...
import type { Embedder } from "./openai";
...
export interface EmbedPassResult {
  ...
  reused: number;
  /** hx.embeddings rows newly inserted (post ON CONFLICT). */
  written: number;
}
...
export async function runEmbedPass(deps: RunEmbedPassDeps): Promise<EmbedPassResult> {
  ...
  // an empty tool row must not be claimed). No lock is held past this read.
  ...
  if (candidates.length === 0) {
    return { claimed: 0, openaiTexts: 0, requests: 0, reused: 0, written: 0 };
  }
  ...
  // 3. REUSE — vectors already stored under an identical content_hash (different
  // turn, same text). Conversational text is mostly session-unique, so this
  // rarely fires across sessions, but it makes a re-embed within a pass free.
  // Bun.SQL expands `IN ${sql(array)}` to a parameterized list ($1,$2,…) — a
  // bare `= ANY(${array})` would CSV-join the array into a malformed literal.
  const reuseRows = (await sql`
    SELECT DISTINCT ON (content_hash) content_hash AS hash, embedding::text AS emb
    FROM hx.embeddings
    WHERE content_hash IN ${sql(uniqueHashes)} AND embedding IS NOT NULL
  `) as Array<{ hash: string; emb: string }>;
  const reuseByHash = new Map<string, string>(reuseRows.map((r) => [r.hash, r.emb]));

  // 4. EMBED the rest via OpenAI, off-lock, batched + concurrency-limited.
  ...
  await Promise.all(
    chunk(needHashes, batchSize).map((batch) =>
      limiter(async () => {
        ...
        batch.forEach((h, i) => {
          if (vectors[i]) newByHash.set(h, vectors[i]);
        });
      }),
    ),
  );

  // 5. INSERT … ON CONFLICT DO NOTHING. The unique index is the write fence.
  let written = 0;
  let reused = 0;
  for (const p of prepared) {
    ...
    if (literal === null) continue; // OpenAI failed for this hash — retried next pass.
    if (reuseLit) reused += 1;
    const inserted = (await sql`
      INSERT INTO hx.embeddings (owner_kind, owner_id, model, dim, embedding, content_hash)
      VALUES ('turn', ${p.id}::uuid, ${embedder.model}, ${embedder.dimensions}, ${literal}::vector, ${p.hash})
  ...
  return { claimed: candidates.length, openaiTexts, requests, reused, written };
}
...
export interface EmbedWorker {
  ...
  /** Stop the scheduler, drain an in-flight pass, and close the DB handle. */
  stop(): Promise<void>;
}
...
export function createEmbedWorker(options: EmbedWorkerOptions): EmbedWorker {
  ...
  async function tick(): Promise<void> {
    ...
    try {
      ...
      if (result.written > 0) logger?.info?.("embed pass", { ...result });
    } catch (err) {
  ...
  return {
    ...
    async runOnce() {
      const sql = ensureSql();
      if (!sql) return { claimed: 0, openaiTexts: 0, requests: 0, reused: 0, written: 0 };
      return runEmbedPass(passDeps(sql));
    },
    async stop() {
      stopped = true;
      if (timer) {
      ...
      for (let i = 0; running && i < 200; i += 1) await sleep(25);
      if (sqlHandle) {
