...
-- looking right: records lost from the MIDDLE of a canonical (the lane stays
-- seq-dense and byte-covering) and watermarks stamped larger than what was
-- actually indexed. Such a session reads as healthy, is never selected, and so
-- is never re-examined by anything.
--
-- TWO timestamps, not one, and the distinction is load-bearing.
--
--   deep_verified_at   the row was PROVEN whole. Drives the backlog: while any
--                      live row is NULL here, the corpus is not proven.
