 ...
 *  (MC-2522) — here and in the gateway's session.json metadata artifact. */
...
export async function ingestCommit(db: HxDb, input: IngestCommitInput): Promise<void> {
  ...
  await db.transaction(async (tx) => {
    ...
    // MC-2522: on append advance monotonically (max over existing + this chunk's
...
export async function ingestAgentCommit(db: HxDb, input: IngestAgentCommitInput): Promise<void> {
  ...
  await db.transaction(async (tx) => {
    ...
    // MC-2522: monotonic on append, authoritative on replace (mirrors the parent lane).
